...
public class HWEncoder  implements Runnable{
   ...
   private static final int NUM_FRAMES = 30;               // 2 seconds of video
   ...
   public byte[] mVideoBuffer = null;
   public byte[] mVideoFrameBuffer = null;
   // largest color component delta seen (i.e. actual vs. expected)
   private int mLargestColorDelta;
   private final static int MAXPACKETSIZE = 1200;
   ...
   private BufferedOutputStream outputStream;
     ...
     */
   	public void writeVideoChunk(byte[]  videoBuffer) {
   			...
   			if (videoBuffer == null) {
   				Log.e(TAG, "output H264 Buffer handed to streamer is null!");
   				return;
   			}
   			byte[] h264Buffer = new byte[videoBuffer.length];
    ...
    */
    ...
    /** Wraps testEncodeDecodeVideoFromBuffer(true) */
          ...
          private Throwable mThrowable;
          private HWEncoder mTest;
    
           private BufferToSurfaceWrapper(HWEncoder test) {
                mTest = test;
            }
               ...
               /**
                 * Entry point.
                 */
                public static void runTest(HWEncoder  obj) throws Throwable {
        ...
        private SurfaceToSurfaceWrapper(HWEncoder  test) {
            mTest = test;
         }
           ...
           private void doEncodeDecodeVideoFromBuffer(MediaCodec encoder, int encoderColorFormat,
                 ...
                 byte[] frameData = new byte[mWidth * mHeight * 3 / 2];
                    ...
                    while (!outputDone) {
                             ...
                             if (inputBufIndex >= 0) {
                                 ...
                                 if (generateIndex == NUM_FRAMES) {
                                    ...
                                    if (inputBuf.capacity() >= frameData.length) {
                                    		Log.d(TAG,  "buffer rezied to hold correct size");
                                    } else{
                                  		fail("buffer not correct size to fit a frame");
                                    }
                                     inputBuf.clear();
                                     ...
                                     fail("encoderOutputBuffer " + encoderStatus + " was null");
                                 	  ...
                                 	  if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                                 		  ...
                                 		  if (decoderConfigured) {
                                 			  fail("Not expecting decoderConfig on subsequent frames only the first!");
                                 		  }
                                 		  MediaFormat format =
                                 			  ...
                                 			  Log.d(TAG, "decoder configured (" + info.size + " bytes)");
                                 	  			...
                                 	  			if (decoderConfigured) {
                                 	  			  Log.d(TAG, "decoder property configured to block until avialable");
                                 	  				...
                                 	  				fail("Decoder buffer input buffer not configured to block unitl avialable!");
                                 	  			}
                     ...
                     if (VERBOSE) 
                    	 	Log.d(TAG, "decoded " + checkIndex + " frames at "
             ...
             private boolean checkFrame(int frameIndex, MediaFormat format, ByteBuffer frameData) {
              ...
              // e.g. Nexus 4 0x7FA30C03 OMX_QCOM_COLOR_FormatYUV420PackedSemiPlanar64x32Tile2m8ka
                   int colorFormat = format.getInteger(MediaFormat.KEY_COLOR_FORMAT);
                    ...
                    if (mWidth != cropWidth) {
                    		fail("Width is not the same as cropWidth");
                    }
                    if (mHeight !=  cropHeight) {
                   		fail("Width is not the same as cropWidth");
                    }
